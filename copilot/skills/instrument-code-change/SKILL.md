...
description: >
  ...
  log levels, structured logging, metrics, traces, spans, telemetry, monitoring,
  instrumentation, or observability, asks to instrument a service, endpoint,
  handler, job, worker, or consumer, asks what to measure or what a dashboard or
  alert should cover, asks about swallowed errors or production error reporting,
  or is diagnosing a production failure with too little signal (for example "add
  logging here", "instrument this", "what should I be measuring", "why can't I
  tell what this is doing", "there's nothing in the logs"). Also use it when a
  change adds a service boundary, background job, queue consumer, or outbound
  dependency to production code, since that is where instrumentation gets
  decided. It picks the smallest signal that answers a real operational
  question, follows the repository's existing telemetry over any better default,
  and keeps secrets, PII, and unbounded cardinality out of telemetry.
---
